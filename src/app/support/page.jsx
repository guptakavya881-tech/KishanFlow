  ...
  useEffect(() => {
    if (!loading) {
      ...
      if (role === 'admin') {
        ...
        router.replace('/buyer/support');
      } else if (role === 'supplier') {
        router.replace('/supplier/support');
      } else {
        router.replace('/farmer/support');
      }
    }
  }, [user, loading, router]);
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-[#faf9f5]">
      ...
      <p className="text-xs font-bold text-slate-500 uppercase tracking-wider">
        Directing to KishanFlow Support Desk...
      </p>
    </div>
  );
}
