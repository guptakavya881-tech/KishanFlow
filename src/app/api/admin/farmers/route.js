...
import { getAdminFarmers, getAdminFarmerDetails, getDatabase } from '@/lib/db';
...
export async function GET(request) {
  try {
    ...
    if (farmerId) {
      const details = getAdminFarmerDetails(parseInt(farmerId, 10));
      if (!details) {
    ...
    const farmers = getAdminFarmers({ search });
    ...
    console.error('Error in /api/admin/farmers:', err);
